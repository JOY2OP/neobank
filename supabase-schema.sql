...
-- Long-lived provider credentials stay outside the public API schema. The
-- service role can access them only through the narrow functions below.
create table private.provider_connection_secrets (
  provider_code text not null references public.providers(code) on delete restrict,
  organization_id uuid not null references public.organizations(id) on delete restrict,
...
create view public.payment_request_status
with (security_invoker = true)
as
select
  pr.*,
...
create view public.current_standing_orders
with (security_invoker = true)
as
...
create view public.business_account_activity
with (security_invoker = true)
as
select
...
-- Current breaks come only from the latest run for each provider/date. Older
-- runs remain immutable history and still contribute first-seen aging.
create view public.latest_reconciliation_breaks
with (security_invoker = true)
as
with latest_run as (
...
begin
  ...
  if v_id is null then
    select id into v_id
    from public.standing_order_events
    where idempotency_key = p_idempotency_key;
  end if;
  return v_id;
end;
...
grant select on table
  ...
  public.current_standing_orders,
  public.business_account_activity,
  public.latest_reconciliation_breaks
to service_role;
    ...
    where n.nspname = 'public'
      and p.proname = any(array[
       'account_balance_as_of', 'statement_lines',
        'save_provider_connection_secret', 'get_provider_connection_secret',
