  ...
  return (
    <AppShell user={user}>
      ...
      <div className="core-loop-list">
        ...
        <Step number="3" title="Issue a Lithic sandbox card" done={cardDone} blocked={!fundingDone || state.modes.lithic !== "sandbox"} detail={cardDone ? `Lithic card ${state.card.provider_card_id} (•••• ${state.card.last4}) is open.` : !fundingDone ? "Wait for the Increase funding transfer to settle first." : state.modes.lithic !== "sandbox" ? "LITHIC_MODE is simulated. Set it to sandbox and add a Lithic sandbox API key." : "Issue a virtual card through Lithic's sandbox."}>
          {isOwner ? <Link className="button button-secondary" href="/app/cards">Issue Lithic card</Link> : <p className="core-instruction">Switch to Sarah Chen to issue the card.</p>}
        </Step>

        <Step number="4" title="Authorize $50, then clear $73.40" done={settlementDone} blocked={!cardDone || state.modes.lithic !== "sandbox"} detail={settlementDone ? `Settlement ${state.settlement.provider_settlement_id} booked ${formatUsd(state.settlement.amount_cents)} on value date ${state.settlement.value_date}.` : authDone ? `Authorization ${state.authorization.provider_authorization_id} has an active hold. Clear it for a different amount.` : "Lithic's transaction simulator creates the card-network authorization; Corgi records its hold before settlement."}>
          {isOps ? <ActionForm action={authDone ? coreSettleCardAction : coreAuthorizeCardAction} submitLabel={authDone ? "Settle $73.40" : "Authorize $50.00"} className="core-action" /> : <p className="core-instruction">Switch to Maya Patel after the Lithic card exists.</p>}
        </Step>
        ...
        <Step number="6" title="Survive a reversed card settlement" done={state.reversal} blocked={!outboundDone || !settlementDone || state.modes.lithic !== "sandbox"} detail={state.reversal ? "The original journal entry remains immutable; an equal and opposite correction was appended on the original value date." : !outboundDone ? "Complete the maker-checker outbound payment first." : "Simulate a Lithic return and append—not mutate—the accounting reversal."}>
          {isOps ? <ActionForm action={coreReverseSettlementAction} submitLabel="Return in Lithic" className="core-action" /> : <p className="core-instruction">Switch to Maya Patel to trigger the Lithic return.</p>}
        </Step>

        <Step number="7" title="Reconcile the Lithic processor file" done={reconDone} blocked={!state.reversal} detail={reconDone ? `Run ${state.reconciliation.id} completed with ${state.breaks.length} currently open break(s).` : !state.reversal ? "Append the reversal before reconciling the processor file." : "Generate the processor row from Lithic's clearing reference and compare it with the immutable ledger posting."}>
